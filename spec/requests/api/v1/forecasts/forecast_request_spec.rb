...
RSpec.describe 'Forecast Request' do
  describe 'GET /api/v1/forecast?location=?' do
    ...
    xit 'returns [:data] with keys [id,type,attributes]', :vcr do
    ...
    xit 'returns attributes via OpenWeather API', :vcr do
      expect(@response[:data][:attributes]).to have_key :current_weather
      expect(@response[:data][:attributes]).to be_a Hash
    end

    xit 'returns Current Weather data', :vcr do
    ...
    xit 'returns Daily Weather data', :vcr do
    ...
    xit 'returns Hourly Weather data', :vcr do
