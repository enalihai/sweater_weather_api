...
RSpec.describe Coordinate do
  it 'can use a hash from the facade/service pattern as data' do
    ...
    expect(coords).to_not respond_to :providedLocation
    expect(coords).to_not respond_to :adminArea3
    expect(coords).to_not respond_to :adminArea6
    expect(coords).to_not respond_to :link_id
  end
end
